  ...
  try {
    ...
    if (!image) {
      return res.status(400).json({
        ...
        example: "POST /api/banana-ai/enhance with body: { \"image\": \"base64string\", \"prompt\": \"improve quality\" }"
      });
    }
    ...
    const resultUrl = await helper.enhanceImage(base64Data, prompt);
...
router.post("/enhance-url", async (req, res) => {
  try {
    ...
    if (!url) {
      return res.status(400).json({
        ...
        example: "POST /api/banana-ai/enhance-url with body: { \"url\": \"https://example.com/image.jpg\", \"prompt\": \"improve quality\" }"
      });
    }
    ...
    console.error("Banana AI URL Error:", error.message);
    return res.status(500).json({
      status: false,
      error: error.message || "Internal server error"
    });
...
module.exports = {
  ...
  type: "post",
  url: `${global.t || "http://localhost:3000"}/api/banana-ai/enhance`,
  logo: "https://aienhancer.ai/favicon.ico",
  category: "tools",
  info: "Enhance images using AI Enhancer with custom prompts",
  router
};
