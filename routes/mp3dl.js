...
class YTDownloader {
  ...
  async dl({ url, quality = "128", trim = false, startT = 0, endT = 0 }) {
    ...
    try {
      ...
      if (error.response) {
        throw new Error(`Server responded with an error: ${error.response.status} - ${JSON.stringify(error.response.data)}`);
...
router.get("/download", async (req, res) => {
  try {
    const { url, quality, trim, startT, endT } = req.query;
    ...
    const result = await downloader.dl({
      url,
      quality: quality || "128",
      trim: trim === "true" || trim === "1",
      startT: parseInt(startT) || 0,
      endT: parseInt(endT) || 0
    });
...
module.exports = {
  path: "/api/ytdownloader",
  name: "YouTube Audio Downloader",
  type: "get",
  url: `${global.t || "http://localhost:3000"}/api/ytdownloader/download?url=https://youtube.com/watch?v=ABC123&quality=128`,
  logo: "https://www.youtube.com/favicon.ico",
  category: "download",
  info: "Download YouTube videos as MP3 audio files with various quality options (64-320 kbps)",
  router
};
