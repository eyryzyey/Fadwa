 ...
 try {
   ...
   if (!image) {
     return res.status(400).json({
       status: false,
       error: "Missing required parameter: image (URL)"
     });
   }
   ...
   if (image.startsWith('http://') || image.startsWith('https://')) {
     ...
     imgBuffer = Buffer.from(imageResponse.data);
   } else {
     return res.status(400).json({
       status: false,
       error: "Invalid image parameter. Must be a valid URL starting with http:// or https://"
     });
   }
