...
const gameChoices = [
  { id: "0", text: "Iya" },
  { id: "1", text: "Tidak" },
  { id: "2", text: "Tidak tahu" },
  { id: "3", text: "Mungkin" },
  { id: "4", text: "Mungkin tidak" },
];
...
class Akinator {
  constructor(region = "id") {
    ...
    this.region = region;
    const proxyBaseURL = `https://${this.region}.akinator.com`;
  ...
  parseHTMLData(htmlString) {
    const data = {};
    try {
      const sessionMatch = htmlString.match(
        /<input[^>]*id=["']session["'][^>]*value=["']([^"']*)["'][^>]*>/i
      );
      if (sessionMatch && sessionMatch[1]) {
        data.session = sessionMatch[1];
      }
      const signatureMatch = htmlString.match(
        /<input[^>]*id=["']signature["'][^>]*value=["']([^"']*)["'][^>]*>/i
      );
      if (signatureMatch && signatureMatch[1]) {
        data.signature = signatureMatch[1];
      }
      const stepMatch = htmlString.match(
        /<input[^>]*id=["']step["'][^>]*value=["']([^"']*)["'][^>]*>/i
      );
      if (stepMatch && stepMatch[1]) {
        data.step = stepMatch[1];
      }
      const questionMatch = htmlString.match(
        /<div[^>]*id=["']question-label["'][^>]*>([^<]*)</i
      );
      if (questionMatch && questionMatch[1]) {
        data.question = questionMatch[1].trim();
      }
      const akitudeMatch = htmlString.match(
        /<img[^>]*id=["']akitude["'][^>]*src=["']([^"']*)["'][^>]*>/i
      );
      ...
      data.choices = [];
      const choicePattern =
        /<div[^>]*class=[^>]*li-game-mobile[^>]*data-index=["'](\d+)["'][^>]*>([^<]*)</g;
      let choiceMatch;
  ...
  parseJSONResponse(responseText) {
    ...
    try {
      ...
      try {
        ...
        const descriptionPropositionMatch = responseText.match(
          /"description_proposition"\s*:\s*"([^"]*)"/
        );
        if (descriptionPropositionMatch)
          data.description_proposition = descriptionPropositionMatch[1];
  ...
  async create() {
    ...
    try {
      ...
      return {
        ...
        akitude: parsedData.akitude
          ? `${this.proxyClient.defaults.baseURL}${parsedData.akitude}`
          : null,
        choices: choices,
  ...
  async step({ answer }) {
    ...
    try {
      ...
      const requestBody = `step=${this.currentStep}&progression=${this.progression}&sid=1&cm=false&answer=${answer}&step_last_proposition=&session=${encodeURIComponent(
        this.session
      ...
      console.log(
        `Received next question. Step: ${this.currentStep}, Progression: ${this.progression}%`
      );
      return {
        finished: false,
        question: data.question || `Pertanyaan langkah ${this.currentStep}`,
        akitude: data.akitude
          ? `${this.proxyClient.defaults.baseURL}/assets/img/akitudes_520x650/${data.akitude}`
          : null,
        step: this.currentStep,
        progression: this.progression,
        choices: choices,
  ...
  async cancelAnswer() {
    ...
    try {
      const requestBody = `step=${this.currentStep}&progression=${this.progression}&sid=1&cm=false&session=${encodeURIComponent(
        this.session
      )}&signature=${encodeURIComponent(this.signature)}`;
  ...
  async continueGame() {
    ...
    try {
      const requestBody = `step=${this.currentStep}&sid=1&cm=false&progression=${this.progression}&session=${encodeURIComponent(
        this.session
      )}&signature=${encodeURIComponent(this.signature)}&forward_answer=1`;
...
router.get("/regions", async (req, res) => {
  try {
    ...
    console.error("Akinator Regions Error:", error);
...
router.post("/start", async (req, res) => {
  try {
    ...
    const aki = new Akinator(region || "id");
    const response = await aki.create();
...
router.post("/step", async (req, res) => {
  try {
    ...
    if (sessionData.cookieJar) {
      akiAnswer.cookieJar = new Map(Object.entries(sessionData.cookieJar));
    }

    const response = await akiAnswer.step({ answer: parseInt(answer) });
    ...
    sessionData.cookieJar = Object.fromEntries(akiAnswer.cookieJar);
    sessionData.finished = response.finished || false;
...
router.post("/cancel", async (req, res) => {
  try {
    ...
    if (sessionData.cookieJar) {
      akiCancel.cookieJar = new Map(Object.entries(sessionData.cookieJar));
    }

    const response = await akiCancel.cancelAnswer();

    sessionData.step = akiCancel.currentStep;
    sessionData.progression = akiCancel.progression;
    sessionData.cookieJar = Object.fromEntries(akiCancel.cookieJar);
    sessionData.updatedAt = new Date();
    ...
    console.error("Akinator Cancel Error:", error);
    return res.status(500).json({
      status: false,
      error: error.message || "Internal Server Error",
    });
...
router.post("/continue", async (req, res) => {
  try {
    ...
    if (sessionData.cookieJar) {
      akiContinue.cookieJar = new Map(Object.entries(sessionData.cookieJar));
    }

    const response = await akiContinue.continueGame();

    sessionData.step = akiContinue.currentStep;
    sessionData.progression = akiContinue.progression;
    sessionData.cookieJar = Object.fromEntries(akiContinue.cookieJar);
    ...
    console.error("Akinator Continue Error:", error);
    return res.status(500).json({
      status: false,
      error: error.message || "Internal Server Error",
    });
...
router.get("/status", async (req, res) => {
  try {
    ...
    console.error("Akinator Status Error:", error);
    return res.status(500).json({
      status: false,
      error: error.message || "Internal Server Error",
    });
...
router.delete("/delete", async (req, res) => {
  try {
    ...
    console.error("Akinator Delete Error:", error);
    return res.status(500).json({
      status: false,
      error: error.message || "Internal Server Error",
    });
...
router.get("/list", async (req, res) => {
  try {
    ...
    console.error("Akinator List Error:", error);
    return res.status(500).json({
      status: false,
      error: error.message || "Internal Server Error",
    });
...
module.exports = {
  path: "/api/akinator",
  name: "Akinator Game API",
  type: "get",
  url: `${global.t || "http://localhost:3000"}/api/akinator/start`,
  logo: "https://play-lh.googleusercontent.com/3cT4w4R8f7v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8v8",
  category: "games",
  info: "API untuk bermain game Akinator dengan dukungan multiple regions dan session management",
  router,
};
