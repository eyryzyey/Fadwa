...
router.get("/detail", async (req, res) => {
  try {
    ...
    if (!response.status) {
      return res.status(404).json({
        status: false,
        error: response.error || "Detail fetch failed",
        result: null
      });
    }
...
module.exports = {
  path: "/api/chordindonesia",
  name: "Chord Indonesia Search",
  type: "get",
  url: `${global.t || "http://localhost:3000"}/api/chordindonesia/search?q=berkley`,
  logo: "https://www.chordindonesia.com/favicon.ico",
  category: "search",
  info: "Search and get guitar chords from Chord Indonesia",
  router
};
