...
class WikiClient {
  ...
  parseComponents($) {
    ...
    try {
      $(".infobox tr, .infobox_v2 tr, table.sidebar tr").each((i, row) => {
        const $row = $(row);
      ...
      if ($containers.length > 0) {
        $containers.each((i, el) => {
          ...
          if (paragraphs.length > 0) {
            sections.push({ heading: heading, content: paragraphs.join("\n\n") });
          }
        });
      }
  ...
  async search({ query, limit = 5, detail = false, related = false }) {
      ...
      for (const item of pages) {
        ...
        if (related) {
          try {
            ...
            entry = { ...entry, related_articles: relatedItems };
          } catch (e) {
...
router.get("/search", async (req, res) => {
  try {
    const { query, limit, detail, related } = req.query;
    ...
    const data = await api.search({
      ...
      related: related === "true" || related === "1",
    });
    ...
    console.error(`[ERR] Endpoint: ${error.message}`);
...
router.get("/page", async (req, res) => {
  try {
    ...
    const api = new WikiClient();
    const data = await api.page(title.trim());
    ...
    console.error(`[ERR] Page Endpoint: ${error.message}`);
    return res.status(500).json({
      status: false,
      error: error.message || "Terjadi kesalahan saat memproses.",
    });
...
module.exports = {
  path: "/api/wikipedia-id",
  name: "Wikipedia Indonesia API",
  type: "get",
  url: `${global.t || "http://localhost:3000"}/api/wikipedia-id/search?query=indonesia&limit=5`,
  logo: "https://upload.wikimedia.org/wikipedia/commons/thumb/8/80/Wikipedia-logo-v2.svg/1200px-Wikipedia-logo-v2.svg.png",
  category: "search",
  info: "API untuk mencari dan mengambil data artikel Wikipedia Bahasa Indonesia dengan dukungan pencarian, detail halaman, dan artikel terkait",
  router,
};
